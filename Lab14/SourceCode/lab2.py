    ...
    for type in range(4):
        ...
        for sz in selections:
            ...
            for calc in range(calculations):
                ...
                z_Q.append((np.quantile(distrib, 0.25) + np.quantile(distrib, 0.75)) / 2)
            ...
            table_rows.append(['D^1/2 * (k = 1)',
                               np.around(np.std(mid), decimals=6),
                               np.around(np.std(medx), decimals=6),
                               np.around(np.std(z_R), decimals=6),
                               np.around(np.std(z_Q), decimals=6),
                               np.around(np.std(z_tr), decimals=6)])

        # TODO: Может сделать удобней, дабы не копировать значения вручную?
        print(tabulate(table_rows, headers=fields, floatfmt='.6f', tablefmt='latex'));
